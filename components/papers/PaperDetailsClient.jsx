...
const StatCard = ({ icon: Icon, label, value, accent = false }) => (
  <div
    className={`flex flex-col gap-1.5 rounded-2xl px-5 py-4 ${
      accent ? "bg-teal-700 text-white" : "bg-slate-50 border border-slate-100"
    }`}
  >
    <div
      className={`flex items-center gap-2 text-xs font-medium tracking-wide uppercase ${
        accent ? "text-teal-200" : "text-slate-400"
      }`}
    >
      <Icon size={13} />
      {label}
...
export default function PaperDetailsClient({ paper }) {
          ...
          {!isPending && !isPremium && (
            <div className="flex items-center gap-3 rounded-2xl border border-slate-200 bg-white px-5 py-3.5 shadow-sm">
              ...
              <p className="text-sm text-slate-600">
                <span className="font-semibold text-slate-800">{paper.freeQuestions} questions</span> are
              ...
              <div className="mt-5 space-y-3">
                  ...
                  <div className="flex items-center gap-3">
                    ...
                    <div>
                      ...
                      <p className="text-xs text-slate-500">
                        {paper.freeQuestions} questions · No account required
                      </p>
                    </div>
                  </div>
                ...
                <div
                  className={`flex items-center justify-between rounded-2xl px-5 py-4 ${
                    isPremium ? "bg-teal-50 border border-teal-100" : "bg-slate-50"
                  }`}
                >
                  <div className="flex items-center gap-3">
                    <div
                      className={`flex h-9 w-9 items-center justify-center rounded-xl border ${
                        isPremium ? "bg-teal-700 border-teal-700" : "bg-white border-slate-200"
                      }`}
                    >
